...
def select_highlight_segments(
        ...
        min_gap_sec:     float = 15.0,   # FIXED: was 2s, now 15s so nearby frames merge
        segment_pad_sec: float = 3.0,    # FIXED: was 1s, now 3s for context
        min_segment_dur: float = 8.0,    # NEW:   drop segments shorter than 8 seconds
        max_segments:    int   = 15      # NEW:   cap at 15 segments maximum
    ...
    # Drop segments that are too short (removes tiny 2-second fragments)
    merged = [seg for seg in merged if (seg[1] - seg[0]) >= min_segment_dur]
